...
- Produces: `class RecentEvents` с конструктором `(capacity: number, lifetimeMs: number, authorCount: number)`, методами `push(path: number, author: number, atMs: number): void`, `forEach(nowMs: number, visit: (path: number, author: number, strength: number) => void): void`, `clear(): void`
...
План исполнен полностью, но текст разошёлся с кодом: находки ревью меняли замысел
по ходу. Авторитетен git, не этот документ. Существенные расхождения:

- **`RecentEvents.activeAuthors` удалён.** Он считал события мёртвых путей, для
  которых ни луча, ни значка не рисуется, и к концу среза оказался методом с
  правильным именем и неправильной семантикой, который никто не звал. Число
  видимых авторов теперь получается побочным продуктом того же прохода, что
  строит цели, — в `web/render/activity.ts`. Не возвращайте его: в срезе 5 для
  панели фильтров понадобится счётчик, и соблазн взять готовое имя будет велик.
- **Покадровый вывод вынесен из точки входа** в `web/render/activity.ts` чистой
  функцией и покрыт тестами: внутри замыкания он не проверялся ничем, кроме
  строки статуса, и подмена источника цвета луча прошла бы незамеченной.
- **Оба конца луча разрешаются в координаты в одном месте.** Раньше начало было
  координатами, а конец — идентификатором пути, и отрисовка независимо
  перепроверяла маску живых. В срезе 5 подмена пути на представителя свёрнутой
  папки — правка одной строки в `deriveActivity`.
- **Оттенки значков ограничены полосами, вычисленными из палитры узлов.** Без
  этого один автор сливался с файлами `.py`: 170° против 170.7°.
- **Палитра и хэш строки вынесены отдельно** (`web/render/palette.ts`,
  `src/util/hash.ts`): модуль цвета автора не должен зависеть от отрисовки, а
  один алгоритм не должен существовать в двух копиях.
- **Поле авторов сбрасывается вместе с буфером** при перемотке, иначе значок
  летел к новой цели 2.8 секунды при жизни луча в 1.2.
