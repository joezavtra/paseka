  ...
  const result = await page.evaluate(
    async ({ windowMs, minSamples, tolerancePx, deadlineMs }) => {
      ...
      for (;;) {
        ...
        // Скользящее окно по времени: не роняем ниже minSamples, даже если это
        // раздувает окно сверх windowMs, — недобор образцов внутри windowMs
        // опаснее, чем окно чуть длиннее заявленного (см. докблок функции).
        while (recent.length > minSamples && t - recent[0]!.t > windowMs) recent.shift();
...
test('карточка узла и поиск работают в собранном приложении', async ({ page }) => {
  ...
  // ниже, — плюс stableCentroid зовётся пять раз за прогон (первичная
  // стабилизация, до и после Enter к найденному узлу, до и после Enter по
  // образцу без совпадений) с дедлайном в 30 с каждый — ещё до 150 с. Итого
  // худший случай — 210 с (30 + 30 + 150). На практике каждый вызов
  // stableCentroid возвращается за доли секунды, как только дерево и правда
  // устоялось, но без щедрого собственного таймаута тест на нагруженной
  // машине упёрся бы в общий предел раньше, чем сработает частный, и вместо
  // внятной причины («центр масс не остановился» или «CLI не ответил») дал
  // бы бесполезное сообщение о таймауте теста — берём 280 с, с запасом
  // сверх посчитанных 210 с.
