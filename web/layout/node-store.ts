...
export interface StoreUpdate {
  /** Рисуемая маска; индекс — идентификатор пути. Авторитетный источник для хранилища. */
  active: Uint8Array;
...
export class NodeStore {
  ...
  applyUpdate(update: StoreUpdate): StoreNode[] {
    ...
    // переиспользовать его под следующий кадр (маска теперь — пересечение
    // живости и видимости, и main.ts собирает её именно так). Хранилище с
    // псевдонимом молча видело бы чужие правки.
    ...
    for (let path = 0; path < this.active.length; path++) {
      ...
      nodes.push(this.known.get(path) ?? this.spawn(path));
    }
