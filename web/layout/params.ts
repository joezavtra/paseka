...
export interface LayoutParams {
  ...
  folderFill: number;
  /** Угловой зазор между конусами соседних подпапок и вокруг ребра вверх, градусы. */
  coneGap: number;
  /** Доля круга под подпапки; остаток круга остаётся файлам. */
  branchBudget: number;
  /** Сила разведения налезших конусов. */
  coneStrength: number;
...
export const LAYOUT_PARAM_SPECS: readonly ParamSpec[] = [
  ...
  },
  {
    ...
    max: 60,
    step: 5,
    hint: 'Сколько градусов держать между соседними подпапками и вокруг ребра вверх: туда уходит связь к родителю, и поддерево там перекрыло бы её.',
  },
  {
    key: 'branchBudget',
    ...
    max: 1,
    step: 0.05,
    hint: 'Какую долю круга делят между собой подпапки. Остаток достаётся файлам: чем меньше доля, тем дальше подпапкам приходится отойти, чтобы уместиться, и тем шире сцена.',
  },
  {
    key: 'coneStrength',
    label: 'Разведение подпапок',
    min: 0,
    max: 1,
    step: 0.05,
    hint: 'Разводит подпапки, оказавшиеся на одном направлении из родителя. Ноль — углами снова не управляет ничто, и рёбра начинают пересекаться.',
  },
  {
    key: 'coneMaxStep',
    label: 'Шаг поворота',
    ...
    max: 200,
    step: 2,
    hint: 'Сколько пикселей за тик разрешено проехать самой дальней точке поворачиваемого поддерева. Защита от рывков у больших поддеревьев.',
  },
  {
    key: 'groupCohesion',
    label: 'Сборка папки',
