...
export class Camera {
  ...
  /**
   * Автоматическое вписывание рисуемых узлов. Зовётся на каждом сообщении
   ...
   *
   * `onManualControl`, если дан, зовётся на каждый жест, который отбирает
   * камеру у автовписывания: на каждый поворот колеса и на каждое перемещение
   * курсора при перетаскивании — то есть там же и столько же раз, сколько
   * зовётся `takeManualControl()`. Одиночный клик без движения камеру не
   * трогает и колбэк не зовёт. Повторные вызовы — норма, колбэк обязан быть
   * идемпотентным. Нужен вызывающему, у которого есть собственное
