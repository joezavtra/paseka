...
/**
 * Фон, на котором лежит сцена. Единственный источник истины в коде: им
 * красятся инициалы значка автора (web/render/scene.ts), от него же считается
 * контраст значков в тесте (tests/web/avatar.test.ts) и он же ограничивает
 * снизу светлоту значка (web/render/avatar.ts). Копия в разметке
 * (web/index.html) неизбежна — CSS не читает модуль, — и там стоит ссылка
 * сюда: менять цвет нужно в этих двух местах, а не в четырёх.
 */
